...
{
  if (isAllocated(ID)) {
    if ((Alarm[ID].onTickHandler != NULL)) {
...
bool TimeAlarmsClass::isRepeating(AlarmID_t ID){
    if(isAllocated(ID)){
...
void TimeAlarmsClass::setOneShot(AlarmID_t ID, bool isOneShot) {
    if(isAllocated(ID)) {
        Alarm[ID].Mode.isOneShot = isOneShot;    
    }
}
void TimeAlarmsClass::setPeriod(AlarmID_t ID, dtAlarmPeriod_t period){
    if(isAllocated(ID)){
